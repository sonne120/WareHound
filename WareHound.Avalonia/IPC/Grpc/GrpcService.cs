...
public class GrpcService : BackgroundService, IHostedGrpcService
{
        ...
        ILoggerService logger)
    {
        ...
        // Subscribe to settings changes using ReactiveUI MessageBus
        MessageBus.Current.Listen<GrpcEnabledChangedEvent>()
            .Subscribe(OnGrpcSettingsChanged)
            .DisposeWith(_disposables);
    }

    private void OnGrpcSettingsChanged(GrpcEnabledChangedEvent e)
    {
        lock (_connectionLock)
        {
            _serverAddress = e.Settings.ServerAddress;

            if (e.Settings.Enabled && !_isEnabled)
            {
                _isEnabled = true;
                _ = ConnectAsync();
            }
            else if (!e.Settings.Enabled && _isEnabled)
            {
                _isEnabled = false;
                _ = DisconnectAsync();
            }
