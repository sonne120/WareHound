...
using WareHound.Avalonia.Services.BackgroundJobs;

namespace WareHound.Avalonia.Services;

public class NativeSnifferService : ISnifferService, IDisposable
{
    private readonly ISnifferInterop _snifferInterop;
    private readonly ILoggerService _logger;
    private readonly IBackgroundJobs<SnapshotStruct>? _backgroundJobs;
    ...
    public NativeSnifferService(
        ISnifferInterop snifferInterop,
        ILoggerService logger,
        IBackgroundJobs<SnapshotStruct>? backgroundJobs = null)
    {
        _snifferInterop = snifferInterop ?? throw new ArgumentNullException(nameof(snifferInterop));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _backgroundJobs = backgroundJobs;

        _logger.LogDebug("NativeSnifferService: Initialized");
    }
    ...
    public void LoadDevices()
    {
        try
        {
            ...
            _logger.LogDebug("Loading network devices via SharpPcap...");
            Devices.Clear();

            // Use SharpPcap for reliable cross-platform device enumeration
            var captureDevices = CaptureDeviceList.Instance;
            _logger.Log($"Found {captureDevices.Count} network devices");
    ...
    public async Task LoadDevicesAsync(TimeSpan timeout, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Loading network devices asynchronously via SharpPcap...");

            using var timeoutCts = new CancellationTokenSource(timeout);
            using var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, timeoutCts.Token);

            IReadOnlyList<ICaptureDevice> captureDevices;
            try
            {
                // Use SharpPcap for reliable cross-platform device enumeration
                captureDevices = await Task.Run(() => CaptureDeviceList.Instance, linkedCts.Token);
            }
    ...
    private void InitializeCapture(int deviceIndex)
    {
        ...
        // Create Windows Event for synchronization with C++ sniffer
        ...
        if (OperatingSystem.IsWindows() && _eventHandle != null && !_eventHandle.IsInvalid)
        {
    ...
    private void PipeReaderLoop()
    {
        ...
        while (_isCapturing && !(_cts?.IsCancellationRequested ?? true))
        {
            try
            {
                ...
                // Read header first
                int bytesRead = _pipeClient.Read(headerBuffer, 0, headerSize);
                ...
                {
                    _logger.LogDebug($"Incomplete read: {bytesRead} of {headerSize} bytes");
                }
            }
            catch (Exception ex)
            {
    ...
    private void ProcessPacketBuffer(byte[] buffer)
    {
        ...
        try
        {
            ...
            _backgroundJobs?.BackgroundTaskGrpc.Enqueue(snapshot);
